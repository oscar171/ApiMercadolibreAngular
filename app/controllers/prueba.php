    ...
    try{
        switch ($x['topic']) 
          ...
          case'payments':
                    ...
                    $sql="SELECT id_orden FROM ventas WHERE id_orden='".$result['body']->order_id."'";
                    $bol=$bd->ejecutar($sql);
                    if(mysql_num_rows($bol>0))
                    {
                    $sql="UPDATE ventas SET status='".$result3['body']->status."', mount='".$result['body']->total_paid_amount."' WHERE id_orden='".$result['body']->order_id."'";
                    $respon=$bd->ejecutar($sql);
                    }
                    else
                    {
